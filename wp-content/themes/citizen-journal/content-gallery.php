	...
	<div class="entry-content post_content">
		<?php if ( post_password_required() ) : ?>
			...
			<?php else : ?>
				...
				<p><em><?php printf( _n( 'This gallery contains <a %1$s>%2$s photo</a>.', 'This gallery contains <a %1$s>%2$s photos</a>.', $total_images, 'citizen-journal' ),
						'href="' . get_permalink() . '" title="' . sprintf( esc_attr__( 'Permalink to %s', 'citizen-journal' ), the_title_attribute( 'echo=0' ) ) . '" rel="bookmark"',
	...
	<footer class="entry-meta">
		<?php if ( 'post' == get_post_type() ) : // Hide category and tag text for pages on Search ?>
			<?php
				...
				$categories_list = get_the_category_list( __( ', ', 'citizen-journal' ) );
				if ( $categories_list && citizenjournal_categorized_blog() ) :
			?>
			<span class="cat-links">
				<?php printf( __( 'Posted in %1$s', 'citizen-journal' ), $categories_list ); ?>
			</span>
			<span class="sep"> | </span>
			...
			<?php
				/* translators: used between list items, there is a space after the comma */
				$tags_list = get_the_tag_list( '', __( ', ', 'citizen-journal' ) );
				if ( $tags_list ) :
			?>
			<span class="tag-links">
				<?php printf( __( 'Tagged %1$s', 'citizen-journal' ), $tags_list ); ?>
			</span>
			<span class="sep"> | </span>
