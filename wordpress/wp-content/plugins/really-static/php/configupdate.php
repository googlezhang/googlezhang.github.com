...
if (isset ( $_POST ["strid"] )) {
	...
	if ($_POST [setexpert])
		...
		if ($_POST ['strid'] == "rs_destination") {
			...
			update_option ( 'rs_designremote', $_POST ['rs_designremote'] );
		}
...
RS_LOG( substr ( $_POST ["refreshurl"], strlen ( loaddaten ( "rs_remoteurl" ) ), - 1 ));
RS_LOG(   loaddaten ( "rs_remoteurl" ));

	if (rs_nonpermanent)
		$mm = really_static_make_to_wp_url( substr ( $_POST ["refreshurl"], strlen ( loaddaten ( "rs_remoteurl" ) ), - 1 ) );
	else
		$mm = str_replace ( array(loaddaten ( "rs_remoteurl" ),loaddaten ( "localurl" )), array("",""), $_POST ["refreshurl"] );
